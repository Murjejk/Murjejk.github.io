...
// Firebase-konfiguration (flyttad utanför DOMContentLoaded för att vara tillgänglig för initiell app-instans)
const firebaseConfig = {
  apiKey: "AIzaSyCvrXL_n-YNgtxwXegG0BzkHu9_CJUPiDU",
  authDomain: "training-d1d9d.firebaseapp.com",
  projectId: "training-d1d9d",
  storageBucket: "training-d1d9d.firebasestorage.app", // Korrigerad enligt modern konfig
  messagingSenderId: "813923749374",
  appId: "1:813923749374:web:7e301c4970d77893f7c4af",
  measurementId: "G-KPV09FMJB7" // Inkluderad då den fanns i AI-snippet
};

// Initiera Firebase-app (utanför DOMContentLoaded för att vara globalt tillgänglig)
const app = initializeApp(firebaseConfig);
// Hämta Authentication service
const auth = getAuth(app);
// Om du planerar att använda Analytics
// const analytics = getAnalytics(app);

// API-URL (utanför DOMContentLoaded, då det är en global konstant)
...
// ==============================================================================
// FUNKTIONER SOM KALLAS FRÅN HTML ONCLICK MÅSTE GÖRAS GLOBALA
...
// Toggle exercises in pass (refereras dynamiskt via li.onclick i loadPassMenu/loadMuscleGroups)
...
// Prefill form med exercise (kallas från dynamiskt genererad HTML)
...
// Vilo-timer
let restTimerAnimation; // Måste vara global eller scope-level
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // LOGIN
  // ==========================
  if (loginBtn) { // Kontrollera att elementet finns innan eventlistener
    loginBtn.addEventListener("click", () => {
      const email = document.getElementById("email").value;
      const password = document.getElementById("password").value;
      loginError.textContent = "";

      signInWithEmailAndPassword(auth, email, password) // Använd den modulära funktionen
        .then(userCredential => {
  ...
  // ==========================
  onAuthStateChanged(auth, user => { // Använd den modulära funktionen
    if (user) {
  ...
  async function loadMuscleGroups() {
    ...
    // Tomma innehållet endast om det finns
    if (container) {
    ...
    try {
      ...
      for (const [muscle, exercises] of Object.entries(muscles)) {
        ...
        exercises.forEach(ex => {
          ...
          exLi.textContent = ex;
          // Använd global funktion för prefillExercise och loadExerciseHistory
          exLi.onclick = (ev) => {
            ev.stopPropagation();
        ...
        li.onclick = () => {
          ...
          toggleExercises(li); // Använd den globala funktionen
        };
  ...
  function loadPassMenu() {
    ...
    if (!container) return; // Se till att containern finns
    ...
    passData.forEach(pass => {
      ...
      pass.exercises.forEach(ex => {
        ...
        li.onclick = () => prefillExercise(ex.name, ex.muscle); // Använd den globala funktionen
